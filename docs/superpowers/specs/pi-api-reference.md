...
**`discover_berdl_collections.py` → `beril discover`:** NO `--berdl-proxy`. `--output` (default `ui/config/berdl_collections_snapshot.json`), `--base-url` (default `https://hub.berdl.kbase.us/apis/mcp`), `--max-databases`, `--skip-schemas`, `--include-non-user-facing`. On-cluster uses `berdl_notebook_utils` (no token); off-cluster REST POST with `Bearer` token. Writes JSON snapshot atomically to file (NOT stdout); stdout = summary line. Exit 0 / 2 (RuntimeError off-cluster no-token). Per-item errors accumulate, don't fail.
