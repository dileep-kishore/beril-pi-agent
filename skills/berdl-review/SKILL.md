...
- **Data leakage & evaluation integrity** — hunt the silent failures: train/test leakage (target / feature / look-ahead / group), selection bias (non-representative subsetting, survivorship), metric misuse (wrong metric, accuracy on an imbalanced target, p-hacking / no multiple-comparison correction, scoring on tuned data), and weak/cherry-picked baselines or a missing held-out set. Read the cell `outputs` (split sizes, class balances, the metric computed), not just the prose; name the cell and the check that would rule each in or out.
